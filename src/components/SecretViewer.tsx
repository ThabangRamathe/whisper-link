...
import { Eye, Clipboard, AlertTriangle, Lock, Home } from "lucide-react";
      ...
      <div className="min-h-screen flex items-center justify-center px-6 bg-background text-foreground">
        <div className="max-w-lg w-full">
          <div className="rounded-xl p-8 text-center shadow-[0_20px_60px_rgba(6,2,10,0.6)] bg-surface">
            <div className="mx-auto w-12 h-12 rounded-full flex items-center justify-center mb-4 bg-danger">
      ...
      <div className="max-w-3xl w-full">
        <div className="rounded-xl p-8 shadow-[0_20px_60px_rgba(6,2,10,0.6)] bg-surface">
          <div className="max-w-xl mx-auto text-center">
            <div className="mx-auto w-12 h-12 rounded-full flex items-center justify-center mb-4" style={{ backgroundColor: 'rgba(0,0,0,0.15)' }}>
            ...
            {!revealed ? (
              <>
                <button
                  ...
                  disabled={loading}
                  className="w-full inline-flex items-center justify-center gap-2 rounded-md px-6 py-3 text-sm font-medium text-white shadow-md disabled:opacity-60 btn-elevate bg-accent"
                >
                  {loading ? "Revealing..." : <><Eye className="h-4 w-4" /> Reveal Secret</>}
                </button>
                <p className="mt-4 text-xs text-muted">The link will become invalid the moment you click &quot;Reveal&quot;.</p>
              ...
              <>
                <div className="mt-6">
                  <div className="relative">
                    <textarea readOnly value={secret ?? ""} className="w-full h-40 rounded-md border p-4 bg-background border-border text-foreground" />
                    <button
                      onClick={async () => {
                        try {
                  ...
                  <p className="mt-4 text-xs text-muted">The link will become invalid the moment you clicked &quot;Reveal&quot;.</p>
                </div>
              </>
            )}
          </div>
